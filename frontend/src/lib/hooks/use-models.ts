...
import { CreateModelRequest, ModelDefaults, ModelTestResult, ThinkingLevel } from '@/lib/types/models'
...
export function useUpdateModelThinking() {
  const queryClient = useQueryClient()
  const { toast } = useToast()
  const { t } = useTranslation()

  return useMutation({
    mutationFn: ({ modelId, thinkingLevel }: { modelId: string; thinkingLevel: ThinkingLevel | null }) =>
      modelsApi.updateThinking(modelId, thinkingLevel),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: MODEL_QUERY_KEYS.models })
      toast({
        title: t('common.success'),
        description: t('models.thinkingSaved'),
      })
    },
    onError: (error: unknown) => {
      toast({
